...
impl Piece {
    pub(crate) fn new(name: PieceName, color: Color) -> Self {
        let p = (name as usize) << 1 | color as usize;
        p.into()
    }
...
mod piece_tests {
    ...
    fn test_piece_color_conversion() {
        assert_eq!(Color::from(0usize), Color::White);
        assert_eq!(Color::from(1usize), Color::Black);
    }
}
